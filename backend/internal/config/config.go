...
func (c *Config) LoadDBConfig() error {
	...
	if c.DB.MaxOpenConns > math.MaxInt32 || c.DB.MaxOpenConns < 0 {
		return fmt.Errorf("max open connections value %d out of range for int32", c.DB.MaxOpenConns)
	}
	if c.DB.MaxIdleConns, err = parseInt(getEnv("DB_MAX_IDLE_CONNS", "25")); err != nil {
		return err
	}
	if c.DB.MaxIdleConns > math.MaxInt32 || c.DB.MaxIdleConns < 0 {
		return fmt.Errorf("max idle connections value %d out of range for int32", c.DB.MaxIdleConns)
	}
